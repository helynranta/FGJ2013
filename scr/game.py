...
class Paddle(pygame.sprite.Sprite):
	def __init__(self,vari):
		...
		self.image = pygame.image.load("img/char01.png")
		self.rect = self.image.get_rect()

class Bottle(pygame.sprite.Sprite):
	def __init__(self,vari,polku):
		pygame.sprite.Sprite.__init__(self)

		self.vari = ""
		self.image = pygame.image.load(polku)
		self.rect = self.image.get_rect()
		if polku == "img/beer_green.png":
			self.vari = "green"
		else:
			self.vari = "orange"
...
class Moomin:
    ...
    def update(self,screen):
        self.stand.blit(screen,[self.x,self.y])
